"""
This script provides utilities to launch webcam, capture/record video, show rectangles & text on screen.
...
from framesandoflow import image_crop, images_crop, frames_show, OpticalFlow, frames2flows, flow2colorimage, \
    flows2colorimages


def video_start(device=0, tuResolution=(320, 240), nFramePerSecond=30):
    """
    Initialises the camera and returns its video stream.
    :param device: device ID to use.
    :param tuResolution: Display resolution.
    :param nFramePerSecond: FPS of the captured video.
    :return: Video Capture Stream.
    """
...
def rectangle_text(arImage, sColor, sUpper, sLower=None, tuRectangle=(224, 224)):
    ...
    :param sColor: Color of text.
    :param sUpper: Upper text.
    :param sLower: Lower text.
    :param tuRectangle: Dimensions of the rectangle.
    :return: Edited frame.
    """
...
def video_show(oStream, sColor, sUpper, sLower=None, tuRectangle=(224, 224), nCountdown=0):
    ...
    while True:
        ...
        arFrame = rectangle_text(arFrame, sColor, s, sLower, tuRectangle)
...
def frame_show(oStream, sColor: str, sText: str, tuRectangle=(224, 224)):
    ...
    oFrame = rectangle_text(oFrame, sColor, sText, "", tuRectangle)
...
def unittest_videocapture():
    ...
    fFPS = oStream.get(cv2.CAP_PROP_FPS)
    sResults = ""
    while True:
...
def unittest_opticalflow_fromcamera():
    ...
    while True:
        ...
        if key == ord(' '):
            # countdown n sec
            video_show(oStream, "orange", "Recording starts in ", sLower=None, tuRectangle=(224, 224), nCountdown=3)

            # record video for n sec
            fElapsed, arFrames, _ = video_capture(oStream, "red", "Recording ", tuRectangle=(224, 224), nTimeDuration=5,
                                                  bOpticalFlow=False)
            print(
